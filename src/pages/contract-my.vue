<template>
    <q-page class="q-pa-lg">
        ...
        <div class="row q-mt-lg">
          <div class="col">
            <q-btn icon="playlist_add" label="导 入" color="primary" @click="imports" />
            <q-btn icon="save" label="备 份" color="secondary" @click="backup" />
            <q-btn icon="add" label="添加合约" color="primary" />
          </div>
        </div>
    </q-page>
</template>
  ...
  methods: {
    ...
    imports () {
      dialog.showOpenDialog({
        title: '选择合约备份文件',
        filters: [
          {name: 'Json File', extensions: ['json', 'JSON']}
        ],
        properties: [
          'openFile'
        ]
              ...
              items.forEach((item) => {
                if (item._id && item.name && item.contractAddress) {
                  ...
                  if (_.isUndefined(doc)) {
                    ...
                    db.contracts.insert(item)
                    count++
                  }
                }
              })
              this.$q.notify({message: '成功导入 ' + count + ' 个合约.', type: 'positive'})
            } else {
